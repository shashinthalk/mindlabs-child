...
|---|---|---|---|---|---|
| `template-home.php` | Meridian Home | `concept-b-meridian_1.html` (theme root) | Yes | **Code-side shape updated 2026-09-02; live Page ID 8 row still holds the OLD shape until synced via the new "Sync Template Updates" meta box button (see functions.php entry, project.json) — the template's own `??` fallbacks already cover the gap, so the page renders correctly either way** | No longer standalone as of 2026-08-30 — now calls `get_header()`/`get_footer()` like every other template, delegating to `header.php`/`footer.php` + `template-parts/site-header.php`/`site-footer.php` (GUIDELINES.md §9) instead of its own inline `<header>`/`<footer>` markup. The `nav`/`footer` keys this page's own JSON used to carry are gone (that content now lives in the site-wide Site Content JSON, `hex_get_site_content()` — see §9's own backfill status); this page's `hex_get_page_content()` JSON keeps `site`/`hero`/`logo_strip`/`services`/`platform`/`results`/`brands`/`quote`/`compliance`/`paths`/`contact` (note: `site.phone`/`site.email` remain here too, read independently by the Contact section). UPDATED 2026-09-02: `logo_strip.names` (a hardcoded competitor/partner brand list) replaced with `logo_strip.shortcode` (?? `[hex_partner_logos]`), rendered via `do_shortcode()` — a real shortcode registered under that tag later will render automatically, no template change needed. `compliance.badges` (three text badges) replaced with `compliance.image_url`/`image_alt`, rendered as a single `<img class="compliance-badges-img">` (new class, `site-theme.css`) — default image is this theme's own new `assets/images/compliance-badges-placeholder.svg`. `brands.cards[].rating` (a literal `'★★★★★'` string) removed — no `<img>` ever existed before the card titles in this template, so nothing else needed removing there; a real logo image before each card title is a separate future task. Assigned to Page ID 8 ("Home"), the static front page. |
